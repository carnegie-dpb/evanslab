...
/**
 * Run through all the SNP files in the given directory and accumulate the counts of homozygous and heterozygous samples per SNP per group.
 ...
 * minDepth = minimum read depth for a SNP to be counted for a given sample.
...
public class SNPZygosity {
    ...
    public static void main(String[] args) {
        ...
        try {
            ...
            for (int i=aMin; i<=aMax; i++) {
                ...
                if (file.exists()) {
                    ...
                    while((line=reader.readLine())!=null) {
                        ...
                        String key = rec.chr+"\t"+String.format("%10d", rec.pos1)+"\t"+rec.ref+"\t"+rec.alt; // SNP key, allele-specific
                        // filtering
                        boolean keep = true;
            ...
            // header line
            System.out.println("contig\tposition\tref\talt\tAreads\tBreads\tAhom\tAhet\tlnAOR\tBhom\tBhet\tlnBOR\tlnBOR-lnAOR");

            // merge the A and B keys=SNPs into a single sorted set, since some are only in one map and some are only in the other
            ...
            for (String key : keySet) {
                ...
                // have to have at least minSamples from one of the counts
                if (aHom>=minSamples || aHet>=minSamples || bHom>=minSamples || bHet>=minSamples) {
                    ...
                    // output
                    System.out.println(key+"\t"+aReads+"\t"+bReads+"\t"+aHom+"\t"+aHet+"\t"+df.format(lnAOR)+"\t"+bHom+"\t"+bHet+"\t"+df.format(lnBOR)+"\t"+df.format(lnBOR-lnAOR));
                }
            }
